...
class BRMemorySaverRNN(BRMemorySaverBase):
    ...
    def add_terminal(
        ...
        if self._intermediate_memory.is_level_1():
            self._game_memory.add(
                o_t=np.copy(self._intermediate_memory.obs_t),
    ...
    def add_non_terminal_experience(
        self, obs_t_before_acted, a_selected_t, legal_actions_list_t
    ):
        self._intermediate_memory.add_experience(
            obs_t=obs_t_before_acted,
