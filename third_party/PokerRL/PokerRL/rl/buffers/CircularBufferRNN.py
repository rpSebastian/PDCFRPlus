...
class CircularBufferRNN(CircularBufferBase):
    ...
    def sample(self, device, batch_size):
        ...
        batch_legal_action_mask_tp1 = torch.from_numpy(
            np.array(batch_legal_action_mask_tp1)
        ).to(device=device)

        batch_legal_action_mask_t = [sample["mask_t"] for sample in samples]
        batch_legal_action_mask_t = torch.from_numpy(
            np.array(batch_legal_action_mask_t)
        ).to(device=device)
        ...
        batch_range_idx = torch.from_numpy(np.array(batch_range_idx)).to(
            dtype=torch.long, device=device
        )

        batch_reward = [sample["rew"] for sample in samples]
        batch_reward = torch.from_numpy(np.array(batch_reward)).to(
    ...
    def state_dict(self):
        return {"games": self._games, "size": self._size, "top": self._top}
