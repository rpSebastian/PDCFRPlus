    ...
    def _apply_grads(self, list_of_grads, net, optimizer, grad_norm_clip=None):
        ...
        for batch in grad_batches:
            for g in self._ray.get(batch):
                if g is not None:
                    list_of_grads.append(
                        self._ray.grads_to_torch(g, device=self._device)
                    )

        # ________________________________________________ apply grads _________________________________________________
        n_grad_sources = len(
            list_of_grads
        )  # have to recompute because the ones that were None are removed now
        if n_grad_sources > 0:
            # reshape and average
            grads = {}
            ...
            if grad_norm_clip is not None:
                torch.nn.utils.clip_grad_norm_(
                    parameters=net.parameters(), max_norm=grad_norm_clip
                )
