...
def get_env_builder(t_prof):
    ...
    return ENV_BUILDER(
        env_cls=get_env_cls_from_str(t_prof.game_cls_str),
        env_args=t_prof.module_args["env"],
    )
...
def get_legal_action_mask_torch(
    n_actions, legal_actions_list, device, dtype=torch.uint8
):
    """
    Args:
...
def batch_get_legal_action_mask_torch(
    n_actions, legal_actions_lists, device, dtype=torch.uint8
):
    """
    ...
    """
    assert isinstance(
        legal_actions_lists[0], list
    ), "need list of lists of legal actions (as ints)!"

    mask = torch.zeros(
        (
            len(legal_actions_lists),
            n_actions,
        ),
        device=device,
        dtype=dtype,
