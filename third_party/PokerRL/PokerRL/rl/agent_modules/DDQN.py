...
class DDQN(_NetWrapperBase):
    def __init__(
        ...
        self._target_net = DuelingQNet(
            env_bldr=env_bldr, q_args=ddqn_args.q_args, device=ddqn_args.device_training
        )
        self._target_net.eval()
        self.update_target_net()

        self._batch_arranged = torch.arange(
            ddqn_args.batch_size, dtype=torch.long, device=self.device
        )
        self._minus_e20 = torch.full(
            (
                ddqn_args.batch_size,
        ...
        self._n_actions_arranged = np.arange(
            self._env_bldr.N_ACTIONS, dtype=np.int32
        ).tolist()
    ...
    def select_br_a(self, pub_obses, range_idxs, legal_actions_lists, explore=False):
        if explore and (np.random.random() < self._eps):
            return np.array(
                [
                    legal_actions[np.random.randint(len(legal_actions))]
                    for legal_actions in legal_actions_lists
                ]
            )
        ...
        with torch.no_grad():
            ...
            for b in range(q.shape[0]):
                illegal_actions = [
                    i
                    for i in self._n_actions_arranged
                    if i not in legal_actions_lists[b]
                ]
    ...
    def from_state_dict(state_dict, env_bldr):
        ddqn = DDQN(
            owner=state_dict["owner"], ddqn_args=state_dict["args"], env_bldr=env_bldr
        )
