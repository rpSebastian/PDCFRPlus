...
class MainPokerModuleRNN(nn.Module):
    ...
    def __init__(
        ...
        self.players_info_starts = [
            player_i_idxs[0] for player_i_idxs in self.env_bldr.obs_players_idxs
        ]
        self.players_info_lens = [
            len(player_i_idxs) for player_i_idxs in self.env_bldr.obs_players_idxs
        ]
        ...
        if mpm_args.use_pre_layers:
            self.cards_fc_1 = nn.Linear(
                in_features=self.env_bldr.obs_size_board + self.env_bldr.priv_obs_size,
                out_features=mpm_args.n_cards_state_units,
            )

            self.cards_fc_2 = nn.Linear(
                in_features=mpm_args.n_cards_state_units,
                out_features=mpm_args.n_cards_state_units,
            )
            ...
            self.table_state_fc = nn.Linear(
                in_features=self.env_bldr.obs_size_table_state
            ...
            self.merge_fc = nn.Linear(
                in_features=mpm_args.n_cards_state_units
                + mpm_args.n_merge_and_table_layer_units,
                out_features=mpm_args.n_merge_and_table_layer_units,
            )

            self.rnn = rl_util.str_to_rnn_cls(mpm_args.rnn_cls_str)(
                input_size=mpm_args.n_merge_and_table_layer_units,
    ...
    def forward(self, pub_obses, range_idxs):
        ...
        if bs > 1:
            seq_lens = torch.tensor(
                [sample.shape[0] for sample in pub_obses],
                device=self.device,
                dtype=torch.int32,
            )
            ...
            _pub_obs = pub_obses
            pub_obses = torch.zeros(
                (max_len, bs, self.pub_obs_size),
                dtype=torch.float32,
                device=self.device,
            )
            ...
            y = torch.nn.utils.rnn.pack_padded_sequence(
                y, lengths=seq_lens, batch_first=False
            )
            ...
            if self.args.sum_step_outputs:
                ...
                y = y[
                    seq_lens - 1,
                    torch.arange(bs, device=self.device, dtype=torch.long),
                    :,
                ].squeeze(dim=0)

            # sort the samples back to their original order
            idx_unsort_obs_t = torch.arange(bs, device=self.device, dtype=torch.long)
            idx_unsort_obs_t.scatter_(
                src=idx_unsort_obs_t.clone(), dim=0, index=idx_shifts
            )
            ...
            pub_obses = (
                torch.from_numpy(pub_obses[0])
                .to(self.device)
                .view(seq_len, bs, self.pub_obs_size)
            )
    ...
    def _feed_through_pre_layers(self, pub_o, priv_o):
        ...
        # Cards Body
        # """""""""""""""
        _cards_obs = torch.cat(
            (
                priv_o,
                pub_o.narrow(dim=-1, start=self.board_start, length=self.board_len),
            ),
            dim=-1,
        )
        ...
        _table_obs = torch.cat(
            ...
            + [
                pub_o.narrow(
                    dim=-1,
                    start=self.players_info_starts[i],
