...
from PokerRL.rl.base_cls.workers.ParameterServerBase import (
    ParameterServerBase as _ParameterServerBase,
)
...
class Local_RLBR_ParameterServer(_ParameterServerBase):
    ...
    def _get_new_net(self):
        return DuelingQNet(
            q_args=self._args.ddqn_args.q_args,
    ...
    def get_eval_ddqn_state_dicts(self):
        ...
        for p in range(self._eval_env_bldr.N_SEATS):
            ddqn = DDQN(
                owner=p, ddqn_args=self._args.ddqn_args, env_bldr=self._eval_env_bldr
            )
