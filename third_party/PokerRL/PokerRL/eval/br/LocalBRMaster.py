...
class LocalBRMaster(EvaluatorMasterBase):
        ...
        for mode in self._t_prof.eval_modes_of_algo:
            ...
            for stack_size_idx, stack_size in enumerate(self._t_prof.eval_stack_sizes):
                ...
                if self._eval_agent.can_compute_mode():
                    expl_p0, expl_p1 = self._compute_br_heads_up(
                        stack_size_idx=stack_size_idx, iter_nr=iter_nr
                    )
                    self._log_results(
                        iter_nr=iter_nr,
                        agent_mode=mode,
            ...
            if self._is_multi_stack:
                self._log_multi_stack(
                    agent_mode=mode,
                    iter_nr=iter_nr,
                    score_total=sum(total_of_all_stacks)
                    / float(len(total_of_all_stacks)),
                )
    ...
    def _compute_br_heads_up(self, stack_size_idx, iter_nr=None, do_export_tree=True):
        ...
        expl_p1 = (
            float(gt.root.exploitability[0]) * self._env_bldr.env_cls.EV_NORMALIZER
        )
        expl_p2 = (
            float(gt.root.exploitability[1]) * self._env_bldr.env_cls.EV_NORMALIZER
        )
