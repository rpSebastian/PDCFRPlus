...
class LocalHead2HeadMaster(EvaluatorMasterBase):
    ...
    def __init__(self, t_prof, chief_handle, eval_agent_cls):
        ...
        self._eval_agents = [
            eval_agent_cls(t_prof=t_prof) for _ in range(self._env_bldr.N_SEATS)
        ]
    ...
    def evaluate(self, iter_nr):
        ...
        if self._is_multi_stack:
            ...
            self._log_multi_stack(
                agent_mode="Head2Head",
                iter_nr=iter_nr,
                score_total=_mean,
    ...
    def _run_eval(self, stack_size):
        winnings = np.empty(
            shape=(self._args.n_hands * self._env_bldr.N_SEATS), dtype=np.float32
        )

        _env = self._eval_env_bldr.get_new_env(
            ...
            for iteration_id in range(self._args.n_hands):
                ...
                while not done:
                    ...
                    if p_id_acting == seat_p0:
                        action_int, _ = self._eval_agents[
                            self._REFERENCE_AGENT
                        ].get_action(step_env=True, need_probs=False)
                        self._eval_agents[1 - self._REFERENCE_AGENT].notify_of_action(
                            p_id_acted=p_id_acting, action_he_did=action_int
                        )
                    elif p_id_acting == seat_p1:
                        action_int, _ = self._eval_agents[
                            1 - self._REFERENCE_AGENT
                        ].get_action(step_env=True, need_probs=False)
                        self._eval_agents[self._REFERENCE_AGENT].notify_of_action(
                            p_id_acted=p_id_acting, action_he_did=action_int
                        )
                    else:
                ...
                # Add Rews
                # """""""""""""""""
                winnings[iteration_id + (seat_p0 * self._args.n_hands)] = (
                    r_for_all[seat_p0] * _env.REWARD_SCALAR * _env.EV_NORMALIZER
                )
