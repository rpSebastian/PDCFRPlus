...
class LocalLBRWorker:
    ...
    def __init__(self, t_prof, chief_handle, eval_agent_cls):
        ...
        self.agent = _AgentWrapper(
            t_prof=t_prof, lbr_args=self.lbr_args, eval_agent_cls=eval_agent_cls
        )
    ...
    def run(self, agent_seat_id, n_iterations, mode, stack_size):
        ...
        self._env = self._eval_env_bldr.get_new_env(
        ...
        if self._eval_env_bldr.env_cls.IS_FIXED_LIMIT_GAME:
            return self._run_limit(
                agent_seat_id=agent_seat_id, n_iterations=n_iterations
            )
        else:
            return self._run_no_limit(
                agent_seat_id=agent_seat_id, n_iterations=n_iterations
            )
            ...
            while not terminal:
                ...
                if p_id_acting == lbr_seat_id:
                    ...
                    if (self.check_to_round is not None) and (
                        ...
                        _asked = (
                            self._env.seats[agent_seat_id].current_bet
                            - self._env.seats[lbr_seat_id].current_bet
                        )
                        _pot_before_action = self._env.get_all_winnable_money()
                        _utility[Poker.CHECK_CALL] = (
                            _wp * _pot_before_action - (1 - _wp) * _asked
                        )
                        ...
                        if Poker.BET_RAISE in self._env.get_legal_actions():
                            ...
                            self.agent.notify_of_action(
                                p_id_acted=lbr_seat_id, action_he_did=Poker.BET_RAISE
                            )
                            ...
                            # ev(state, lbr_a=r)
                            _chips_lbr_puts_in_pot = (
                                _pot_after_raise - _pot_before_action
                            )
                            _ev_if_fold = _pot_before_action
                            _ev_if_not_fold = (_wp_now * _pot_after_raise) - (
                                (1 - _wp_now) * _chips_lbr_puts_in_pot
                            )
                            _utility[Poker.BET_RAISE] = (
                                _fold_prob * _ev_if_fold
                    ...
                    self.agent.notify_of_action(
                        p_id_acted=lbr_seat_id, action_he_did=action_int
                    )
                    ...
                    self.agent_range.update_after_action(
                        action=action_int, all_a_probs_for_all_hands=a_probs_each_hand
                    )
                ...
                if self._env.current_round != old_game_round:
                    self.agent_range.update_after_new_round(
                        new_round=self._env.current_round, board_now_2d=self._env.board
                    )

            total_lbr_winnings[iteration_id] = (
                reward[lbr_seat_id] * self._env.REWARD_SCALAR * self._env.EV_NORMALIZER
            )
            ...
            while not done:
                ...
                if p_id_acting == lbr_seat_id:
                    ...
                    if (self.check_to_round is not None) and (
                        ...
                        # illegal: -1, fold: 0, all other: any float
                        _utility = np.full(
                            shape=2 + n_lbr_bets, fill_value=-1.0, dtype=np.float32
                        )
                    ...
                    if action_int >= 2:
                        ...
                        raise_frac = self.agent.cpu_agent.env_bldr.env_args.bet_sizes_list_as_frac_of_pot[
                            action_int - 2
                        ]
                ...
                if (
                    action_int >= 2
                ):  # step with fraction because agent and LBR have different raise sizes
...
class _AgentWrapper:
    def __init__(self, t_prof, lbr_args, eval_agent_cls):
        self.USE_GPU = (
            t_prof.HAVE_GPU
            and lbr_args.use_gpu_for_batch_eval
        ...
        if self.USE_GPU:
            self.gpu_agent = eval_agent_cls(
                t_prof=t_prof, device=torch.device("cuda:0")
            )
    ...
    def get_action(self, step_env, need_probs):
        if (
            need_probs and self.USE_GPU
        ):  # if need_probs is True, strategy for all hands is computed --> batch
    ...
    def notify_of_action(self, p_id_acted, action_he_did):
        ...
        self.cpu_agent.notify_of_action(
            p_id_acted=p_id_acted, action_he_did=action_he_did
        )
...
class _LBRRolloutManager:
    def __init__(self, t_prof, env_bldr, env, lbr_hand_2d):
        ...
        self._lbr_hand_range_idx = (
            self.env_bldr.lut_holder.get_range_idx_from_hole_cards(
                hole_cards_2d=lbr_hand_2d
            )
        )

        self._board_2d = np.copy(self._env.board)  # still has not-dealt cards
        self._board_1d = self.env_bldr.lut_holder.get_1d_cards(
            self._board_2d
        )  # still has not-dealt cards

        self._cards_dealt = np.array(
            [c for c in self._board_1d if c != Poker.CARD_NOT_DEALT_TOKEN_1D]
        )
        self._possible_cards = np.arange(
            self.env_bldr.rules.N_CARDS_IN_DECK, dtype=np.int32
        )
        self._possible_cards = np.delete(
            self._possible_cards, np.concatenate((self._cards_dealt, self._lbr_hand_1d))
        )

        self._n_cards_to_deal = (
            env_bldr.lut_holder.DICT_LUT_N_CARDS_OUT[self._env.ALL_ROUNDS_LIST[-1]]
            - env_bldr.lut_holder.DICT_LUT_N_CARDS_OUT[self._env.current_round]
        )

        # recursion
        self._build_eq_vecs(
            board_1d=np.copy(self._board_1d),
    ...
    def _build_eq_vecs(self, board_1d, n_cards_to_deal, possible_cards_1d):
        if n_cards_to_deal > 0:
            ...
            handranks = self._env.get_hand_rank_all_hands_on_given_boards(
                boards_1d=board_1d.reshape(1, board_1d.shape[0]),
                lut_holder=self.env_bldr.lut_holder,
            )[0]
    ...
    def get_lbr_checkdown_equity(self, agent_range):
        ...
        if self.t_prof.DEBUGGING:
            ...
            assert np.allclose(
                np.sum(agent_card_probs), self.env_bldr.rules.N_HOLE_CARDS, atol=0.0001
            )
        cards_dealt = np.array(
            [c for c in self._board_1d if c != Poker.CARD_NOT_DEALT_TOKEN_1D]
        )
