...
class EvaluatorMasterBase(WorkerBase):
    ...
    def __init__(
        self, t_prof, eval_env_bldr, chief_handle, eval_type, log_conf_interval=False
    ):
        """
        Args:
        ...
        self._exp_name_total, self._exp_names_conf = self._create_experiments(
            self_name=eval_type
        )

        if self._is_multi_stack:
            self._exp_name_multi_stack = {
                eval_mode: self._ray.get(
                    self._ray.remote(
    ...
    def pull_current_strat_from_chief(self):
        ...
        """
        w, self._chief_info = self._ray.get(
            self._ray.remote(
                self._chief_handle.pull_current_eval_strategy, self._chief_info
            )
        )
        return w
    ...
    def _log_results(
        ...
        self._ray.remote(
            ...
            self._exp_name_total[agent_mode][stack_size_idx],
            graph_name,
            iter_nr,
            score,
        )
        ...
        if self._log_conf_interval:
            ...
            self._ray.remote(
                self._chief_handle.add_scalar,
                self._exp_names_conf[agent_mode][stack_size_idx][0],
                graph_name,
                iter_nr,
                lower_conf95,
            )
            self._ray.remote(
                self._chief_handle.add_scalar,
                self._exp_names_conf[agent_mode][stack_size_idx][1],
                graph_name,
                iter_nr,
                upper_conf95,
            )

    def _log_multi_stack(
        self, agent_mode, iter_nr, score_total, upper_conf95=None, lower_conf95=None
    ):
        """
        ...
        self._ray.remote(
            self._chief_handle.add_scalar,
            self._exp_name_multi_stack[agent_mode],
            graph_name,
            iter_nr,
            score_total,
        )
        ...
        if self._log_conf_interval:
            ...
            self._ray.remote(
                self._chief_handle.add_scalar,
                self._exp_names_multi_stack_conf[agent_mode][0],
                graph_name,
                iter_nr,
                lower_conf95,
            )
            self._ray.remote(
                self._chief_handle.add_scalar,
                self._exp_names_multi_stack_conf[agent_mode][1],
                graph_name,
                iter_nr,
                upper_conf95,
            )
