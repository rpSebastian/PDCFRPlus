...
class CrayonWrapper:
    ...
    def __init__(
        ...
        self._ray = MaybeRay(
            runs_distributed=runs_distributed, runs_cluster=runs_cluster
        )
    ...
    def export_all(self, iter_nr):
        ...
        if self._path_log_storage is not None:
            path_crayon = ospj(
                self._path_log_storage, str(self._name), str(iter_nr), "crayon"
            )
            path_json = ospj(
                self._path_log_storage, str(self._name), str(iter_nr), "as_json"
            )
            ...
            for e in self._experiments.values():
                ...
                write_dict_to_file_json(
                    dictionary=self._custom_logs, _dir=path_json, file_name="logs"
                )
        ...
        for name, vals_dict in new_v.items():
            for graph_name, data_points in vals_dict.items():
                for data_point in data_points:
                    ...
                    self._experiments[name].add_scalar_value(
                        name=graph_name, step=step, value=val
                    )
