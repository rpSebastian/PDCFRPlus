...
from PokerRL.game._.rl_env.game_rules import (
    BigLeducRules,
    FlopHoldemRules,
    HoldemRules,
    Leduc5Rules,
...
class StandardLeduc(LeducRules, LimitPokerEnv):
    ...
    def __init__(self, env_args, lut_holder, is_evaluating):
        ...
        LimitPokerEnv.__init__(
            self, env_args=env_args, lut_holder=lut_holder, is_evaluating=is_evaluating
        )
...
class Flop5Holdem(FlopHoldemRules, LimitPokerEnv):
    ...
    def _adjust_raise(self, raise_total_amount_in_chips):
        return self.get_fraction_of_pot_raise(
...
class DiscretizedNLHoldemSubGame(DiscretizedNLHoldem):
    ...
    def create_root_env_state(self):
        ...
        board = np.array(
            [self.str_to_id_dict[card_str] for card_str in board]
            + [[-127, -127] for _ in range(5 - len(board))]
        )
