...
import numpy as np
from PokerRL.game._.tree._.nodes import ChanceNode, PlayerActionNode
from PokerRL.game._.tree._.StrategyFiller import StrategyFiller, StrategyFillerHUNL
from PokerRL.game._.tree._.ValueFiller import ValueFiller, ValueFillerHUNL
...
class PublicTree:
    ...
    def __init__(
        ...
        _dir_tree_vis = os.path.join(
            "C:\\" if os.name == "nt" else os.path.expanduser("~/"), "PokerRL_Viz"
        )

        # This is just me being over-cautious to not overwrite someones files...
        _tree_vis_installed = os.path.isdir(_dir_tree_vis) and os.path.isfile(
            os.path.join(_dir_tree_vis, "ALLOWED_TO_WRITE_HERE.dontdelete")
        )
        ...
        self._env = self._env_bldr.get_new_env(
        ...
        self._stop_at_street = (
            max(self._env.ALL_ROUNDS_LIST) + 1
            if stop_at_street is None
            else stop_at_street
        )
    ...
    def build_tree(self):
        ...
        """
        if hasattr(self._env, "root_env_state"):
            # print("load env state")
            self._env.load_state_dict(self._env.root_env_state)
        if hasattr(self._env, "reach_probs"):
            # print("load reach probs")
            reach_probs = self._env.reach_probs
        else:
            reach_probs = np.full(
                shape=(self._env_bldr.N_SEATS, self._env_bldr.rules.RANGE_SIZE),
        ...
        self.root.reach_probs = reach_probs
    ...
    def _build_tree(self, current_node):
        ...
        self._n_nonterm += len(
            [c for c in current_node.children if (not c.is_terminal)]
        )
        ...
        if parent.p_id_acting_next == self.CHANCE_ID:
            ...
            def _make_boards(_new_boards, _board_1d, _idx_to_insert_in, _n_todo):
                if _n_todo > 0:
                    for _c in [
                        _
                        for _ in range(self._env.N_CARDS_IN_DECK)
                        if _ not in _board_1d
                    ]:
                        ...
                        for i in range(_n_todo):
                            _make_boards(
                                _new_boards=_new_boards,
                                _board_1d=_b,
                                _idx_to_insert_in=_idx_to_insert_in + 1,
                                _n_todo=_n_todo - 1,
                            )

                        _new_boards.append(
                            self._env_bldr.lut_holder.get_2d_cards(cards_1d=_b)
                        )

            new_boards = []
            board_1d = self._env_bldr.lut_holder.get_1d_cards(
                cards_2d=_base_env_state[EnvDictIdxs.board_2d]
            )
            _make_boards(
                _new_boards=new_boards,
                _board_1d=board_1d,
                _idx_to_insert_in=self._env_bldr.lut_holder.DICT_LUT_N_CARDS_OUT[
                    self._env_bldr.rules.ROUND_BEFORE[
            ...
            for board in new_boards:
                ...
                _deck_remaining_before_deal = np.copy(
                    child_env_state[EnvDictIdxs.deck]["deck_remaining"]
                )
                for c in board:
                    child_env_state[EnvDictIdxs.deck]["deck_remaining"] = np.delete(
                        _deck_remaining_before_deal,
                        np.where(_deck_remaining_before_deal == c),
                        axis=0,
                    )
            ...
            for action in parent.allowed_actions:
                ...
                if is_terminal:
                    ...
                    env_state[EnvDictIdxs.current_round] = parent.env_state[
                        EnvDictIdxs.current_round
                    ]
                    env_state[EnvDictIdxs.board_2d] = parent.env_state[
                        EnvDictIdxs.board_2d
                    ]
                    env_state[EnvDictIdxs.deck] = copy.deepcopy(
                        parent.env_state[EnvDictIdxs.deck]
                    )
                    is_terminal = True
                ...
                # after action:  New round
                    ...
                    if self._is_debugging:
                        assert (
                            env_state[EnvDictIdxs.current_round]
                            == parent.env_state[EnvDictIdxs.current_round]
                        )
                        assert np.array_equal(
                            env_state[EnvDictIdxs.board_2d],
                            np.copy(parent.env_state[EnvDictIdxs.board_2d]),
                        )
                        assert np.array_equal(
                            env_state[EnvDictIdxs.deck]["deck_remaining"],
                            np.copy(
                                parent.env_state[EnvDictIdxs.deck]["deck_remaining"]
                            ),
                        )
                    is_terminal = False
                    new_round_state = self._env.state_dict()
                    new_round_state[EnvDictIdxs.board_2d] = np.copy(
                        parent.env_state[EnvDictIdxs.board_2d]
                    )
                    new_round_state[EnvDictIdxs.deck] = copy.deepcopy(
                        parent.env_state[EnvDictIdxs.deck]
                    )
    ...
    def _get_children_nodes(self, node):
        if node.is_terminal:
            ...
            return self._get_children_of_action_node(
                parent=node
            )  # -> action or chance or terminal

    def _get_action_as_str(self, action):
        if action is None:
            return "None"
        mapping = {Poker.FOLD: "FOLD", Poker.CHECK_CALL: "CHECK", "CHANCE": "CHANCE"}
    ...
    def _export_for_node_strategy_tree(self, node):
        ...
        if node.parent is None:
            ...
            title = (
                "Player acted last "
                + str(node.p_id_acted_last)
                + " :: Action: "
                + self._get_action_as_str(node.action)
                + " :: Board: "
                + self._env.cards2str(cards_2d=node.env_state[EnvDictIdxs.board_2d])
            )
        ...
        json_table = {
            "text": {
                ...
                "title": title,
                "round": "Round : "
                + Poker.INT2STRING_ROUND[node.env_state[EnvDictIdxs.current_round]],
                # for body
                "main_pot": "Pot : "
                + json.dumps(int(node.env_state[EnvDictIdxs.main_pot])),
                "terminal": ("TERM" if str(node.is_terminal) else "")
                + " "
                + str(node.allowed_actions),
                "side_pots": "SP: "
                + json.dumps([int(i) for i in node.env_state[EnvDictIdxs.side_pots]]),
                "stack_sizes": "Stacks: "
                + json.dumps(
                    [
                        int(s[PlayerDictIdxs.stack])
                        for s in node.env_state[EnvDictIdxs.seats]
                    ]
                ),
                "current_bets": "Bets: "
                + json.dumps(
                    [
                        int(s[PlayerDictIdxs.current_bet])
                        for s in node.env_state[EnvDictIdxs.seats]
                    ]
                ),
                "not_folded": "Playing: "
                + json.dumps(
                    [
                        not s[PlayerDictIdxs.folded_this_episode]
                        for s in node.env_state[EnvDictIdxs.seats]
                    ]
                )
                .replace("true", "1")
                .replace("false", "0")
                + "  Next: "
                + str(node.p_id_acting_next),
                "exploitability": "Exploitability: "
                + (
                    str(node.exploitability)
                    if node.exploitability is not None
                    else "Exploitability not computed"
                )
                + "   ||   BR Action per hand "
                + (
                    ""
                    if node.br_a_idx_in_child_arr_for_each_hand is None
                    else str(
                        [
                            self._get_action_as_str(a)
                            for a in np.array(node.allowed_actions)[
                                node.br_a_idx_in_child_arr_for_each_hand
                            ]
                        ]
                    )
                ),
...
class PublicTreeHUNL(PublicTree):
    def __init__(
        self,
        env_bldr,
        stack_size,
        ...
        super().__init__(
            ...
            stack_size,
            stop_at_street,
            put_out_new_round_after_limit,
            is_debugging,
        )
        self._value_filler = ValueFillerHUNL(tree=self)
        self._strategy_filler = StrategyFillerHUNL(tree=self, env_bldr=self.env_bldr)
