...
class FlatHULimitPokerHistoryWrapper(Wrapper):
    ...
    def _reset_state(self, **kwargs):
        ...
        self._action_history_vector = np.zeros(
            shape=self._action_vector_size, dtype=np.float32
        )
    ...
    def _pushback(self, env_obs=None):
        ...
        if _last_a is not None:
            ...
            idx = self.env_bldr.get_vector_idx(
                round_=self._game_round_last_tick,
                p_id=_last_actor,
    ...
    def print_obs(self, wrapped_obs=None):
        ...
        print()
        print(
            "________________________________________ OBSERVATION HISTORY ________________________________________"
        )
        print()
        self.env.print_obs(
            wrapped_obs[: self.env_bldr.pub_obs_size - self._action_vector_size]
        )
        print()
    ...
    def get_current_obs(self, env_obs=None):
        if env_obs is None:
            return np.concatenate(
                (
                    self.env.get_current_obs(is_terminal=False),
                    self._action_history_vector,
                ),
                axis=0,
            )
        else:
    ...
    def set_to_public_tree_node_state(self, node):
        ...
        add(
            node
        )  # will go from node to parent to parent of parent... to root and reverse the direction for the tree
        # fake step in the internal env to gain current rr state
        self.reset()
        self._reset_state()
        for sd in state_seq:
            self.env.load_state_dict(
                sd, blank_private_info=True
            )  # load cause _pushback uses the env's internal state
            self._pushback()
