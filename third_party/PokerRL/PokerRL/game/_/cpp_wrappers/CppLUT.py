...
class CppLibHoldemLuts(CppWrapper):
    ...
    def get_hole_card_2_idx_lut(self):
        lut = np.full(
            shape=(HoldemRules.N_CARDS_IN_DECK, HoldemRules.N_CARDS_IN_DECK),
            fill_value=-2,
            dtype=np.int16,
        )
    ...
    def get_idx_2_flop_lut(self):
        lut = np.full(
            shape=(self._n_boards_lut[Poker.FLOP], self._n_cards_out_lut[Poker.FLOP]),
            fill_value=-2,
            dtype=np.int8,
        )
    ...
    def get_idx_2_turn_lut(self):
        lut = np.full(
            shape=(self._n_boards_lut[Poker.TURN], self._n_cards_out_lut[Poker.TURN]),
            fill_value=-2,
            dtype=np.int8,
        )
    ...
    def get_idx_2_river_lut(self):
        lut = np.full(
            shape=(self._n_boards_lut[Poker.RIVER], self._n_cards_out_lut[Poker.RIVER]),
            fill_value=-2,
            dtype=np.int8,
        )
