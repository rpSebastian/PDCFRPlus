...
class PokerPlayer:
    ...
    def reset(self):
        if self.IS_EVALUATING or (
            self.stack_randomization_range[0] == 0
            and self.stack_randomization_range[1] == 0
        ):
            self.starting_stack_this_episode = self._base_starting_stack
        else:
            self.starting_stack_this_episode = max(
                self.poker_env.BIG_BLIND,
                np.random.randint(
                    low=self._base_starting_stack
                    - np.abs(self.stack_randomization_range[0]),
                    high=self._base_starting_stack
                    + self.stack_randomization_range[1]
                    + 1,
                ),
            )
    ...
    def player_state(self):
        return (self.seat_id, self.stack, self.folded_this_episode, self.is_allin)
