...
class PokerEnv:
    ...
    RANGE_SIZE = (
        NotImplementedError  # number of possible unique combinations of holecards
    )
    ...
    def __init__(
        ...
        (
            ...
            self.obs_idx_dict,
            self.obs_parts_idxs_dict,
        ) = self._construct_obs_space()
        ...
        self.last_action = (
            None  # list of 3 ints: [action_idx, _raise_amount, player.seat_id]
        )
        self.capped_raise = (
            CappedRaise()
        )  # [happend_this_round, player_that_raised, player_that_cant_reopen]
        self.current_player = (
            None  # PokerPlayer instance of player currently having to get_action
        )
    ...
    def _construct_obs_space(self):
        ...
        if (self.N_SEATS == 2) and self._USE_SIMPLE_HU_OBS:
            ...
            _table_space = [  # (blinds are in obs to give the agent a perspective on starting stack after normalization
                get_new_box(
                    "ante", next_idx, self.N_SEATS
                ),  # .................................... self.ANTE
                get_new_box(
                    "small_blind", next_idx, self.N_SEATS
                ),  # ............................. self.SMALL_BLIND
                get_new_box(
                    "big_blind", next_idx, self.N_SEATS
                ),  # ............................... self.BIG_BLIND
                get_new_box(
                    "min_raise", next_idx, self.N_SEATS
                ),  # ............................... min total raise
                get_new_box(
                    "pot_amt", next_idx, self.N_SEATS
                ),  # ................................. main_pot amount
                get_new_box(
                    "total_to_call", next_idx, self.N_SEATS
                ),  # ........................... total_to_call
                get_new_box(
                    "last_action_how_much", next_idx, self.N_SEATS
                ),  # .................... self.last_action[1]
            ]
            for i in range(
                3
                ...
                _table_space.append(
                    get_discrete(1, "last_action_what_" + str(i), next_idx)
                )
            ...
            for i in range(
                ...
                _table_space.append(
                    get_discrete(1, "last_action_who_" + str(i), next_idx)
                )
            ...
            for i in range(
                ...
                _table_space.append(
                    get_discrete(1, "p" + str(i) + "_acts_next", next_idx)
                )

            for i in range(
                max(self.ALL_ROUNDS_LIST) + 1
            ):  # ...................................... round onehot
                _table_space.append(
            ...
            for i in range(self.N_SEATS):
                ...
                _player_space += [
                    get_new_box(
                        "stack_p" + str(i), next_idx, self.N_SEATS
                    ),  # ..................... stack
                    get_new_box(
                        "curr_bet_p" + str(i), next_idx, self.N_SEATS
                    ),  # .................. current_bet
                    get_discrete(
                        1, "is_allin_p" + str(i), next_idx
                    ),  # ............................ is_allin
                ]
            ...
            for i in range(self.N_TOTAL_BOARD_CARDS):
                ...
                for j in range(
                    ...
                    x.append(
                        get_discrete(
                            1, str(i) + "th_board_card_rank_" + str(j), next_idx
                        )
                    )
                ...
                for j in range(
                    ...
                    x.append(
                        get_discrete(
                            1, str(i) + "th_board_card_suit_" + str(j), next_idx
                        )
                    )
            ...
            _observation_space = spaces.Tuple(
                _table_space + _player_space + _board_space
            )
            ...
            for i in range(self.N_SEATS):
                ...
                _player_space += [
                    ...
                    get_discrete(
                        1, "has_folded_this_episode_p" + str(i), next_idx
                    ),  # ............. folded_this_epis
                    get_discrete(
                        1, "is_allin_p" + str(i), next_idx
                    ),  # ............................ is_allin
                ...
                for j in range(self.N_SEATS):
                    _player_space.append(
                        get_discrete(
                            1, "side_pot_rank_p" + str(i) + "_is_" + str(j), next_idx
                        )
                    )  # . side_pot_rank
    ...
    def _init_from_args(self, env_args, is_evaluating):
        ...
        try:
            self.MAX_CHIPS = (
                sum(a.starting_stack_sizes_list)
                + a.stack_randomization_range[1] * a.n_seats
                + 1
            )
        except TypeError:  # stack size set to None -> Default
            self.MAX_CHIPS = (
                a.n_seats * (self.DEFAULT_STACK_SIZE + a.stack_randomization_range[1])
                + 1
            )
        ...
        if a.scale_rewards:
            try:
                self.REWARD_SCALAR = (
                    float(sum(a.starting_stack_sizes_list)) / float(a.n_seats) / 5
                )
            except TypeError:  # stack size set to None -> Default
    ...
    def _deal_turn(self):
        self.board[
            self.N_FLOP_CARDS : self.N_FLOP_CARDS + self.N_TURN_CARDS
        ] = self.deck.draw(self.N_TURN_CARDS)
    ...
    def _payout_pots(self):
        ...
        if self.N_SEATS == 2:
            ...
            pots = np.array(
                [self.main_pot] + self.side_pots
            )  # appends mainpot as idx 0 to sidepots
            ...
            for e in pot_and_pot_ranks:
                ...
                eligible_players = [
                    p
                    for p in self.seats
                    if p.side_pot_rank >= rank and not p.folded_this_episode
                ]

                num_eligible = len(eligible_players)
                if num_eligible > 0:
                    # side_pot / num_winners could be non-int! if so, we decide randomly who to give the chip to.
                    winner_list = self._get_winner_list(
                        players_to_consider=eligible_players
                    )  # PokerPlayer objects
    ...
    def _assign_hand_ranks_to_all_players(self):
        for player in self.seats:
            player.hand_rank = self.get_hand_rank(
                hand_2d=player.hand, board_2d=self.board
            )
    ...
    def _put_current_bets_into_main_pot_and_side_pots(self):
        if self.N_SEATS == 2:
            ...
            # If there is a difference between biggest and 2nd biggest current_bet, award that to the player.
            _players_sorted_by_bet_in_front = sorted(
                self.seats, key=lambda x: x.current_bet, reverse=True
            )

            dif = (
                _players_sorted_by_bet_in_front[0].current_bet
                - _players_sorted_by_bet_in_front[1].current_bet
            )
            ...
            def _find_next_smallest_bet():
                ...
                for b_idx in range(self.N_SEATS):
                    if current_bets[b_idx] > 0:
                        ...
                        if (
                            next_bet_idx is None
                            or current_bets[b_idx] < current_bets[next_bet_idx]
            ...
            idx_smallest_bet = (
                _find_next_smallest_bet()
            )  # get status after main pot calc

            while idx_smallest_bet is not None:
                current_max_side_pot_rank = max(
                    [p.side_pot_rank for p in self.seats]
                )  # -1 on default.
                side_pot_idx = current_max_side_pot_rank + 1

                side_pot_amount_per_player_in_it = self.seats[
                    idx_smallest_bet
                ].current_bet

                players_not_all_in_after_this_cleanup = [
                    p
                    for p in self.seats
                    if not (
                ...
                for p in self.seats:
                    ...
                    amount_contributed = min(
                        p.current_bet, side_pot_amount_per_player_in_it
                    )
    ...
    def _next_round(self):
        ...
        self.current_round += (
            1  # highly dependant on PokerEnv.""ROUND_NAME"" being sequential ints!
        )
    ...
    def _step(self, processed_action):
        ...
        if processed_action[0] == Poker.CHECK_CALL:
            ...
            if processed_action[1] < self._get_current_total_min_raise():
                ...
                self.capped_raise.player_that_cant_reopen = (
                    self.last_raiser
        ...
        self.last_action = [
            processed_action[0],
            processed_action[1],
            self.current_player.seat_id,
        ]

        # ______________________________________________________________________________________________________________
        # check if should deal next round, rundown or continue to next step in the episode of the poker game

        all_non_all_in_and_non_fold_p = [
            p for p in self.seats if not p.folded_this_episode and not p.is_allin
        ]
        all_nonfold_p = [p for p in self.seats if not p.folded_this_episode]

        # just let next player run in this round
        info = None
        if self._should_continue_in_this_round(
            all_non_all_in_and_non_fold_p=all_non_all_in_and_non_fold_p,
        ...
        # next round
            ...
            if self.current_round == len(self.ALL_ROUNDS_LIST) - 1:
                ...
                if self.RETURN_PRE_TRANSITION_STATE_IN_INFO:
                    info = {
                        "chance_acts": False,
                        "state_dict_before_money_move": self.state_dict(),
                    }
                self._payout_pots()
        ...
        # rundown
            ...
            len(all_nonfold_p) > 1
        ):  # rundown only makes sense if >0 are allin and 1 is not or >2 are allin.
            is_terminal = True
            ...
            if self.RETURN_PRE_TRANSITION_STATE_IN_INFO:
                info = {
                    "chance_acts": False,
                    "state_dict_before_money_move": state_before_payouts,
                }
    ...
    def _get_current_total_min_raise(self):
        ...
        if self.N_SEATS == 2:
            _sorted_ascending = sorted(
                [p.current_bet for p in self.seats]
            )  # 0 is small, 1 is big
            delta = max(_sorted_ascending[1] - _sorted_ascending[0], self.BIG_BLIND)
            return _sorted_ascending[1] + delta

        else:
            current_bets_sorted_descending = sorted(
                [p.current_bet for p in self.seats], reverse=True
            )
            ...
            for i in range(1, self.N_SEATS):
                ...
                delta_between_last_and_before_last = (
                    _largest_bet - current_bets_sorted_descending[i]
                )
    ...
    def _get_new_board(self):
        return np.full(
            (self.N_TOTAL_BOARD_CARDS, 2), Poker.CARD_NOT_DEALT_TOKEN_1D, dtype=np.int8
        )
    ...
    def _get_player_that_has_to_act_next(self):
        ...
        raise RuntimeError(
            "There is no next player. Seems like some more debugging is needed..."
        )
    ...
    def _get_fixed_action(self, action):
        ...
        if _action_idx == Poker.FOLD:
            ...
            if self.IS_FIXED_LIMIT_GAME:
                if (
                    self.n_raises_this_round
                    >= self.MAX_N_RAISES_PER_ROUND[self.current_round]
                ):
                    return self._process_check_call(total_to_call=total_to_call)

            if (
                self.current_player.stack + self.current_player.current_bet
                <= total_to_call
            ...
            raise RuntimeError(
                "invalid action ({}) must be fold (0), call (1), or raise (2) ".format(
                    _action_idx
                )
            )
    ...
    def _process_check_call(self, total_to_call):
        delta_to_call = min(
            total_to_call - self.current_player.current_bet, self.current_player.stack
        )
    ...
    def _process_raise(self, raise_total_amount_in_chips):
        raise_to = self._adjust_raise(
            raise_total_amount_in_chips=raise_total_amount_in_chips
        )
    ...
    def _should_continue_in_this_round(
        self, all_non_all_in_and_non_fold_p, all_nonfold_p
    ):
        ...
        if (
            len(
                [p for p in all_nonfold_p if p.is_allin or p.current_bet == largest_bet]
            )
            == len(all_nonfold_p)
            and len(
                [p for p in all_non_all_in_and_non_fold_p if not p.has_acted_this_round]
            )
            == 0
        ):
            return False
    ...
    def _get_table_state(self, normalization_sum):
        if (self.N_SEATS == 2) and self._USE_SIMPLE_HU_OBS:
            community_state = [
                ...
                self.last_action[1] / normalization_sum
    ...
    def _get_step_reward(self, is_terminal):
        ...
        return [
            (p.stack - p.starting_stack_this_episode) / self.REWARD_SCALAR
            for p in self.seats
        ]
    ...
    def reset(self, deck_state_dict=None):
        ...
        self.last_action = [
            ...
            None,
            None,
        ]  # list of 3 ints: [action_idx, _raise_amount, player.seat_id]
    ...
    def step_raise_pot_frac(self, pot_frac):
        ...
        processed_action = (
            2,
            self.get_fraction_of_pot_raise(
                fraction=pot_frac, player_that_bets=self.current_player
            ),
        )
    ...
    def state_dict(self):
        env_state_dict = {
            ...
            EnvDictIdxs.capped_raise: [
                self.capped_raise.player_that_raised.seat_id,
                None
                if self.capped_raise.player_that_cant_reopen is None
                else self.capped_raise.player_that_cant_reopen.seat_id,
            ]
            if self.capped_raise.happened_this_round
            else None,
    ...
    def load_state_dict(self, env_state_dict, blank_private_info=False):
        ...
        for p in self.seats:
            p.seat_id = env_state_dict[EnvDictIdxs.seats][p.seat_id][
                PlayerDictIdxs.seat_id
            ]
            p.stack = env_state_dict[EnvDictIdxs.seats][p.seat_id][PlayerDictIdxs.stack]
            p.current_bet = env_state_dict[EnvDictIdxs.seats][p.seat_id][
                PlayerDictIdxs.current_bet
            ]
            p.is_allin = env_state_dict[EnvDictIdxs.seats][p.seat_id][
                PlayerDictIdxs.is_allin
            ]
            p.folded_this_episode = env_state_dict[EnvDictIdxs.seats][p.seat_id][
                PlayerDictIdxs.folded_this_episode
            ]
            p.has_acted_this_round = env_state_dict[EnvDictIdxs.seats][p.seat_id][
                PlayerDictIdxs.has_acted_this_round
            ]
            p.side_pot_rank = env_state_dict[EnvDictIdxs.seats][p.seat_id][
                PlayerDictIdxs.side_pot_rank
            ]
            ...
            if blank_private_info:
                ...
                p.hand = np.copy(
                    env_state_dict[EnvDictIdxs.seats][p.seat_id][PlayerDictIdxs.hand]
                )
                p.hand_rank = env_state_dict[EnvDictIdxs.seats][p.seat_id][
                    PlayerDictIdxs.hand_rank
                ]
    ...
    def get_current_obs(self, is_terminal):
        ...
        if is_terminal:
            ...
            )  # terminal is all zero
        normalization_sum = (
            float(sum([s.starting_stack_this_episode for s in self.seats]))
            / self.N_SEATS
        )
        return np.array(
            self._get_table_state(normalization_sum=normalization_sum)
    ...
    def get_legal_actions(self):
        ...
        a = self._get_env_adjusted_action_formulation(
            action=(
                Poker.BET_RAISE,
                1,
            )
    ...
    def get_range_idx(self, p_id):
        ...
        """
        return int(
            self.lut_holder.get_range_idx_from_hole_cards(
    ...
    def get_fraction_of_pot_raise(self, fraction, player_that_bets):
        ...
        _player = (
            player_that_bets
            if isinstance(player_that_bets, PokerPlayer)
            else self.seats[player_that_bets]
        )
        to_call = (
            self._get_biggest_bet_out_there_aka_total_to_call() - _player.current_bet
        )
        pot_after_call = (
            self.main_pot
            + sum(self.side_pots)
    ...
    def render(self, mode="TEXT"):
        ...
        if mode.upper() == "TEXT":
            ...
            print(
                ...
                (
                    Poker.INT2STRING_ROUND[self.current_round]
                    + " - "
                    + str(self.current_player.seat_id)
                    + " acts"
                ).center(15),
                "___________________________________",
            )
            ...
            print(
                (
                    "Last Action:   player_"
                    + str(self.last_action[2])
                    + ": "
                    + str(self.last_action[0])
                    + " "
                    + str(self.last_action[1]).rjust(113)
                ),
                "|   Main_pot: ",
                str(self.main_pot).rjust(7),
            )
            ...
            for p in self.seats:
                ...
                print(
                    (f + "Player_" + str(p.seat_id) + ":").rjust(14) + "stack: ",
                    str(p.stack).rjust(8),
                    "current_bet: ",
                    str(p.current_bet).rjust(8),
                    "side_pot_rank: ",
                    str(p.side_pot_rank).rjust(8),
                    "hand: ",
                    self.cards2str(p.hand).rjust(8),
                    # sidepots
                    " " * 18,
                    "|   Side_pot"
                    + str(p.seat_id)
                    + ": "
                    + str(self.side_pots[p.seat_id]).rjust(6),
                )
    ...
    def print_tutorial(self):
        ...
        print("2 \tRaise")
        print(
            "If you enter 2 for raise, you will be asked what size you want to raise to TOTAL in case you play a "
    ...
    def human_api_ask_action(self):
        while True:
            try:
                action_idx = int(
                    input(
                        "What action do you want to take as player "
                        + str(self.current_player.seat_id)
                        + "?"
                    )
                )
                ...
                print(
                    "You need to enter one of the allowed actions. Refer to the tutorial please."
                )
                continue
            if action_idx not in [Poker.FOLD, Poker.CHECK_CALL, Poker.BET_RAISE]:
                print(
                    "Invalid action_idx! Please enter one of the allowed actions as described in the tutorial above."
                )
                continue
            break
        time.sleep(0.01)
