...
class DiscretizedPokerEnv(_PokerEnv):
    ...
    def __init__(self, env_args, lut_holder, is_evaluating):
        ...
        super().__init__(
            env_args=env_args, lut_holder=lut_holder, is_evaluating=is_evaluating
        )

        self.bet_sizes_list_as_frac_of_pot = sorted(
            env_args.bet_sizes_list_as_frac_of_pot
        )  # ascending
        self.other_bet_sizes_list_as_frac_of_pot = sorted(
            env_args.other_bet_sizes_list_as_frac_of_pot
        )  # ascending
        self.N_ACTIONS = env_args.N_ACTIONS
        self.other_n_actions = env_args.other_n_actions
    ...
    def _get_env_adjusted_action_formulation(self, action_int):
        ...
        if action_int == 1:
            ...
            if self.other_bet_sizes_list_as_frac_of_pot is not None:
                if self.n_raises_this_round == 0:
                    selected = self.get_fraction_of_pot_raise(
                        fraction=self.bet_sizes_list_as_frac_of_pot[action_int - 2],
                        player_that_bets=self.current_player,
                    )
                else:
                    selected = self.get_fraction_of_pot_raise(
                        fraction=self.other_bet_sizes_list_as_frac_of_pot[
                            action_int - 2
                        ],
                        player_that_bets=self.current_player,
                    )
            ...
            if self.uniform_action_interpolation and not self.IS_EVALUATING:
                ...
                if action_int == self.N_ACTIONS - 1:  # if highest bet in repertoire
                    if self.IS_POT_LIMIT_GAME:  # if PL game: max is pot
                        max_amnt = self.get_fraction_of_pot_raise(
                            fraction=1.0, player_that_bets=self.current_player
                        )
                    ...
                    bigger = self.get_fraction_of_pot_raise(
                        fraction=self.bet_sizes_list_as_frac_of_pot[action_int - 1],
                        player_that_bets=self.current_player,
                    )
                ...
                if (
                    min_amnt >= max_amnt
                ):  # can happen. The sampling would always be the same -> save time.
                    return [2, min_amnt]
    ...
    def get_legal_actions(self):
        ...
        if self.other_bet_sizes_list_as_frac_of_pot is not None:
            if self.n_raises_this_round == 0:
                ...
                n_actions = self.other_n_actions
        else:
            n_actions = self.N_ACTIONS
        for a in range(2, n_actions):  # only loops through raises
            adj_a = self._get_env_adjusted_action_formulation(action_int=a)
            fixed_a = self._get_fixed_action(action=adj_a)

            if (
                adj_a[0] != fixed_a[0]
            ):  # if we wanted to raise, but env told us we can not, dont append a raise
                break

            if (adj_a[1] < fixed_a[1]) and a < n_actions:
                _last_too_small = a
            else:
                if _last_too_small is not None:
                    legal_actions.append(
                        _last_too_small
                    )  # was to small, but rounded to minraise & is unique -> append
                    _last_too_small = None

                legal_actions.append(
                    a
                )  # this action might be modified by env, but is legal and unique -> append

            if (
                adj_a[1] > fixed_a[1]
            ):  # if the raise was too big, an even bigger one will yield the same result
                break
    ...
    def print_tutorial(self):
        ...
        for i in range(2, self.N_ACTIONS):
            print(
                i,
                "\tRaise ",
                self.bet_sizes_list_as_frac_of_pot[i - 2] * 100,
                "% of the pot",
            )
    ...
    def human_api_ask_action(self):
        ...
        return self._get_fixed_action(
            self._get_env_adjusted_action_formulation(action_int=action_idx)
        )
