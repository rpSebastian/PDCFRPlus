...
class LeducRules:
    ...
    RANGE_SIZE = PokerRange.get_range_size(
        n_hole_cards=N_HOLE_CARDS, n_cards_in_deck=N_CARDS_IN_DECK
    )
    ...
    ROUND_BEFORE = {Poker.PREFLOP: Poker.PREFLOP, Poker.FLOP: Poker.PREFLOP}
    ROUND_AFTER = {Poker.PREFLOP: Poker.FLOP, Poker.FLOP: None}

    RANK_DICT = {i: str(i + 2) for i in range(N_RANKS)}
    SUIT_DICT = (
        {k: ["a", "b", "c", "d", "e", "f", "g"][k] for k in range(N_SUITS)}
        if N_SUITS < 8
    ...
    def get_hand_rank_all_hands_on_given_boards(self, boards_1d, lut_holder):
        ...
        """
        hand_ranks = np.full(
            shape=(boards_1d.shape[0], LeducRules.RANGE_SIZE),
            fill_value=-1,
            dtype=np.int32,
        )
        for board_idx in range(boards_1d.shape[0]):
            for range_idx in range(LeducRules.RANGE_SIZE):
                hand_ranks[board_idx, range_idx] = self.get_hand_rank(
                    hand_2d=lut_holder.get_2d_hole_cards_from_range_idx(
                        range_idx=range_idx
                    ),
...
class Leduc5Rules(LeducRules):
    """
    Leduc Hold'em with 2 suits and 5 ranks
    """
...
class Leduc9Rules(LeducRules):
    """
    Leduc Hold'em with 2 suits and 9 ranks.
    """
...
class Leduc13Rules(LeducRules):
    """
    Leduc Hold'em with 2 suits and 13 ranks.
    """
...
class HoldemRules:
    ...
    SUIT_DICT = {Poker.CARD_NOT_DEALT_TOKEN_1D: "", 0: "h", 1: "d", 2: "s", 3: "c"}
