...
class _LutGetterBase:
    ...
    def get_range_idx_to_private_obs_LUT(self):
        ...
        lut = np.zeros(
            shape=(self.rules.RANGE_SIZE, D * self.rules.N_HOLE_CARDS), dtype=np.float32
        )
    ...
    def get_n_boards_LUT(self):
        ...
        return {
            r: comb(
                N=self.rules.N_RANKS * self.rules.N_SUITS,
                k=_c[r],
                exact=True,
                repetition=False,
            )
    ...
    def get_n_cards_out_at_LUT(self):
        return {
            ...
            Poker.RIVER: self.rules.N_FLOP_CARDS
            + self.rules.N_TURN_CARDS
            + self.rules.N_RIVER_CARDS,
        }
    ...
    def get_n_board_branches_LUT(self):
        ...
        for r in [_r for _r in self.rules.ALL_ROUNDS_LIST if _r != Poker.PREFLOP]:
            ...
            lut[r] = PokerRange.get_range_size(
                n_hole_cards=_N_CARDS_DEALT_IN_TRANSITION_TO_LUT[r], n_cards_in_deck=nc
            )
        return lut
...
class _LutGetterHoldem(_LutGetterBase):
    def __init__(self, env_cls):
        ...
        self.cpp_backend = CppLibHoldemLuts(
            n_boards_lut=self.get_n_boards_LUT(),
            n_cards_out_lut=self.get_n_cards_out_at_LUT(),
        )

    def get_1d_card_2_2d_card_LUT(self):
        lut = np.full(
            shape=(self.rules.N_CARDS_IN_DECK, 2), fill_value=-2, dtype=np.int8
        )
    ...
    def get_2d_card_2_1d_card_LUT(self):
        lut = np.full(
            shape=(self.rules.N_RANKS, self.rules.N_SUITS), fill_value=-2, dtype=np.int8
        )
        for r in range(self.rules.N_RANKS):
            for s in range(self.rules.N_SUITS):
                lut[r, s] = self.cpp_backend.get_1d_card(
                    card_2d=np.array([r, s], dtype=np.int8)
                )
        return lut
    ...
    def get_card_in_what_range_idxs_LUT(self):
        lut = np.full(
            shape=(self.rules.N_CARDS_IN_DECK, self.rules.N_CARDS_IN_DECK - 1),
            fill_value=-2,
            dtype=np.int32,
        )
...
class _LutHolderBase:
    ...
    def __init__(self, lut_getter):
        ...
        self.LUT_RANGE_IDX_TO_PRIVATE_OBS = (
            self._lut_getter.get_range_idx_to_private_obs_LUT()
        )
        ...
        self.LUT_CARD_IN_WHAT_RANGE_IDXS = (
            self._lut_getter.get_card_in_what_range_idxs_LUT()
        )
        ...
        self.DICT_LUT_CARDS_DEALT_IN_TRANSITION_TO = (
            self._lut_getter.get_n_cards_dealt_in_transition_to_LUT()
        )
    ...
    def get_1d_cards(self, cards_2d):
        ...
        aa = np.copy(cards_2d)
        aa[
            aa == Poker.CARD_NOT_DEALT_TOKEN_1D
        ] = 0  # for robustness against not-dealt tokens
        return np.where(
            cards_2d[:, 0] == Poker.CARD_NOT_DEALT_TOKEN_1D,  # not dealt
    ...
    def get_2d_cards(self, cards_1d):
        ...
        cards_2d[
            np.where(cards_1d == Poker.CARD_NOT_DEALT_TOKEN_1D)
        ] = Poker.CARD_NOT_DEALT_TOKEN_2D.reshape(2)
        return cards_2d
...
class LutHolderHoldem(_LutHolderBase):
    ...
    def get_2d_hole_cards_from_range_idx(self, range_idx):
        ...
        return np.array(
            [self.LUT_1DCARD_2_2DCARD[c1], self.LUT_1DCARD_2_2DCARD[c2]], dtype=np.int8
        )
